...
**Klarstellung (Zeichnen/Flackern):** Die `StateMachine` ruft absichtlich nur `update()` auf dem aktiven Screen auf, kein begleitendes `draw()`. Jeder Screen entscheidet in seinem eigenen `update()`/`onEnter()`, wann tatsächlich neu gezeichnet wird (z. B. `HomeScreen`/`ClockScreen` höchstens 1×/Sekunde für die Uhrzeit, `TaskScreen` nur bei Zustandswechseln, `SnakeScreen` jeden Frame fürs Animationstempo). Ein zusätzlicher, bedingungsloser `draw()`-Aufruf aus dem Hauptloop würde diese Drosselung wirkungslos machen und auf echter Hardware bei den nicht Sprite-gepufferten Screens sichtbar flackern. `SnakeScreen` (und alle anderen Spiele) zeichnen deshalb konsequent über ein `M5Canvas`-Offscreen-Sprite (Abschnitt 3: "Sprite-basiertes Rendering ... für flüssige Pixel-Art-Grafik") statt direkt auf `M5.Display`. **Update (erster echter Hardware-Test, Abschnitt 15 Punkt 17):** `HomeScreen` hielt sich urspruenglich nicht an dieses Muster (mehrere direkte `M5.Display`-Aufrufe je Redraw-Tick) und flackerte auf echter Hardware sichtbar – umgestellt auf dasselbe Canvas-Muster.
...
- **Nachtmodus:** `NightModeService` dimmt den Bildschirm screen-unabhängig zwischen einstellbaren Start-/Endstunden (Default 20–7 Uhr, Wraparound über Mitternacht unterstützt). Ohne "kurz antippen zum Aufhellen" – siehe Abschnitt 16. **Update (Nutzerwunsch "Figur schlaeft zwischen 20:00 und 07:00, in dieser Zeit kann nichts gespielt werden"):** dasselbe Zeitfenster (`Profile::nightStartHour`/`nightEndHour`) sperrt jetzt zusaetzlich das Spielen, unabhaengig vom `nightModeEnabled`-Dimmen-Schalter (neue `nightmodeservice::isNight()`-Abfrage, reine Zeitfenster-Pruefung ohne den Toggle). Durchgesetzt an drei Stellen: `HomeScreen` graut die Spiele-Zone aus und zeigt einen Mond+"Zzz"-Hinweis beim Charakter; `GamesMenuScreen` verweigert den Einstieg und schickt bei bereits offenem Menue automatisch zurueck zu Home, sobald die Nachtstunden beginnen; `PlaytimeTicker` (von allen 10 Spielen genutzt) beendet eine bereits laufende Sitzung, falls die Nachtstunden waehrenddessen beginnen. Aufgaben-Modus/Alltagsfunktionen bleiben bewusst ausgenommen (nur "Spielen" im engeren Sinn ist gemeint).
- **Eltern-PIN:** `PinEntryScreen` (Ziffernblock, kein Text) gated `SettingsScreen`; dort Tageslimit anpassen (±5 Min, 15–180 Min), Bonus-Zeit vergeben (+10 Min), Nachtmodus an/aus, Uhrzeit einstellen (**Update, Abschnitt 15 Punkt 17:** neuer `DateTimeSetScreen` - Jahr/Monat/Tag/Stunde/Minute per Stepper, direkt aus `SettingsScreen` ohne erneuten PIN-Check), PIN ändern (führt zurück zu `PinEntryScreen` im "neuen PIN festlegen"-Modus). Eine UI zum Anpassen der Nachtmodus-Uhrzeiten selbst ist nicht umgesetzt.
...
**Stand:** Alle 6 Phasen (0–5) sind im Code umgesetzt: State-Machine, Storage-Layer mit atomaren Schreibvorgängen, `BootScreen`, `ProfileSetupScreen` (Kind-Profil-Auswahl aus `include/KidProfiles.h`), `HomeScreen` mit echtem Sprite-Charakter (Menschenkind, waechst/trainiert ueber die Stufen, `sdcard/sprites/character/`, Fallback auf Platzhalter-Grafik ohne SD-Karte/Dateien – siehe Abschnitt 4) + Namen + 4-Zonen-Navigation, `SubjectSelectScreen`, `TaskScreen`/`TaskEngine` (alle vier Multiple-Choice-Fächer, mit Spaced Repetition und Schwierigkeitsanstieg), `GedaechtnisScreen` (Karten-Memory/Sequenz-Merkspiel), `GamesMenuScreen` + alle 9 Spiele, `AlltagMenuScreen` (Uhr/Wecker, Timer, Checkliste, Steckbrief, Aussehen/`CharacterCustomizeScreen`, Geburtstags-Countdown/`BirthdayScreen`), `PinEntryScreen` + `SettingsScreen` (Eltern-PIN-geschützt: Tageslimit, Bonus-Zeit, Nachtmodus, Uhrzeit einstellen/`DateTimeSetScreen`, PIN ändern, Web-Sync), `AlarmService` + `NightModeService`, `WebServerService` + `WebSyncScreen` (Fortschrittsansicht, Aufgaben-Verwaltung, Tageslimit, ArduinoOTA). **Update:** inzwischen auf echter Core2-Hardware kompiliert, geflasht und gespielt – siehe Punkt 17 in Abschnitt 15 für die dabei gefundenen und behobenen Laufzeit-Fehler (Home-Screen-Flackern, SD-Karten-Erkennung, Nachtmodus-Default, PIN-Persistenz, fehlende Spiele-Zurück-Buttons, Power-Taste) sowie das jetzt auf maximale Knalligkeit finalisierte Neon-Farbschema plus prozeduraler SNES-/Synthwave-Hintergrund (`RetroBackdrop.*`). Siehe `README.md` für den aktuellen Stand und Build-Anleitung.
...
16. **`CharacterRenderer` liest Sprites selbst statt `drawPngFile(SD, ...)` (Abschnitt 4, zweiter echter Kompilierfehler):** siehe Update-Hinweis in Abschnitt 4 - `SD.open()`/`File::read()` in einen Speicherpuffer statt der dateisystem-generischen M5GFX-Ueberladung, die fuer die eingebaute `SD`-Bibliothek keine `DataWrapperT`-Spezialisierung hat.
...
17. **Erste echte Hardware-Testrunde (Abschnitt 5/6/9/11, neu) – auf einem echten Core2 gefundene und behobene Laufzeit-Fehler:**
    - **Home-Screen-Flackern:** `HomeScreen` zeichnete mehrere Aufrufe nacheinander direkt auf `M5.Display` statt (wie alle Spiele) ueber ein Offscreen-`M5Canvas` und einmaligem `pushSprite()`. Umgestellt auf dasselbe Canvas-Muster; `CharacterRenderer::draw()` bekam dafuer einen expliziten `LovyanGFX* target`-Parameter statt intern fest `M5.Display` zu nutzen.
    - **SD-Karte "nicht gefunden" trotz korrektem FAT32 und korrekt abgelegten Ordnern:** SPI-Bus-Fehlkonfiguration - `SD.begin(cs)` ohne explizites `SPI`-Objekt nutzte nicht die von `M5.begin()` bereits Core2-spezifisch belegten Pins. Fix: `SD.begin(cs, SPI)` in `BootScreen.cpp`.
    - **Display zu dunkel / Nachtmodus faelschlich aktiv:** `nightModeEnabled` war default `true` statt `false` (`ProfileStore.h`/`.cpp`), zusaetzlich konnte eine noch unplausible RTC-Werkszeit (Jahr < 2024, vor dem allerersten Uhrzeit-Stellen) in ein zufaelliges "Nacht"-Fenster fallen. `NightModeService::check()` bekam einen Plausibilitaets-Guard (Jahr < 2024 => normale Helligkeit erzwingen), zusaetzlich zum korrigierten Default.
    - **Eltern-PIN nach Neustart wieder auf Default zurueckgefallen:** `PinEntryScreen` pruefte den Rueckgabewert von `profilestore::save()` beim Setzen eines neuen PIN nicht - schlug der SD-Schreibvorgang fehl (hier: als Folge des oben genannten SPI-Bus-Fehlers), wirkte die PIN-Aenderung nur bis zum naechsten Neustart, ohne dass das sichtbar wurde. `submitIfComplete()` rollt das im Speicher schon geaenderte `guard` jetzt bei einem Speicherfehler zurueck und zeigt eine Fehlermeldung, statt eine nicht dauerhafte Aenderung stillschweigend als erfolgreich zu melden.
    - **Kein Zurueck-/Home-Button in den Spielen:** `SnakeScreen` hatte als einziges der 9 Spiele gar keinen Home-Icon-Handler; die anderen 8 hatten zwar schon einen, aber in einer schwer erkennbaren reinen Weiss-Umriss-Optik. Snake bekam denselben Handler wie die anderen Spiele, alle 9 wurden auf ein einheitliches, deutlich sichtbares goldenes Badge-Icon umgestellt.
    - **Uhrzeit liess sich nicht einstellen:** neuer `DateTimeSetScreen` (Jahr/Monat/Tag/Stunde/Minute per Stepper, Speichern schreibt erst beim Bestaetigen in `M5.Rtc`) als sechste Zeile in `SettingsScreen` ergaenzt (dafuer `kRowHeight` von 38 auf 32px kompaktiert, damit sechs Zeilen weiterhin auf den 240px hohen Screen passen). Kein zusaetzlicher PIN-Check noetig, da `SettingsScreen` selbst schon Eltern-PIN-geschuetzt ist.
    - **Power-Taste ohne Funktion:** `main.cpp::loop()` wertet jetzt `M5.BtnPWR` aus - kurzer Klick schaltet das Display per `M5.Display.sleep()`/`wakeup()` an/aus (dabei werden `NightModeService::check()` und `StateMachine::update()` bewusst ausgesetzt, damit weder ein spaeterer Nachtmodus-Check den manuellen Toggle sofort ueberschreibt noch Beruehrungen auf dem dunklen Screen etwas veraendern oder Spielzeit verbrauchen), Halten ueber 2 Sekunden sichert den Fortschritt (`persistProgress()`) und schaltet das Geraet ueber `M5.Power.powerOff()` komplett aus - bewusst kein `ESP.restart()`/Reset (Nutzerwunsch: "kein Weg um Fortschritt zurueckzusetzen ausser SD-Karte loeschen"). Erneutes Druecken der Power-Taste schaltet ueber den AXP-Power-Chip wieder ein (reiner Hardware-Vorgang, keine Firmware-Logik dafuer noetig) und durchlaeuft `setup()`/`BootScreen` wie bei jedem Kaltstart - der komplette Fortschritt kommt dabei unveraendert von der SD-Karte zurueck.
    - **Farbschema/Grafik-Finalisierung (Nutzerwunsch "knallige Neon-Retro-Farben...SNES/80er-Arcade finalisieren"):** `Theme.h`-Akzentfarben naeher an ihre maximal gesaettigten Grundtoene verschoben (weniger Pastellanteil), Hintergrund/Panel abgedunkelt fuer mehr Kontrast. Zusaetzlich neues `src/core/RetroBackdrop.*`: ein rein aus Theme-Farben und Zeichenprimitiven prozedural gezeichneter "Synthwave"-Hintergrund (Gold-zu-Pink-Sonne uebereinem perspektivischen, zum Horizont hin engmaschiger werdenden Bodengitter) ohne zusaetzliche Bild-Assets - eingebunden in `HomeScreen`, alle drei Icon-Menues (`GamesMenuScreen`/`AlltagMenuScreen`/`SubjectSelectScreen`), `KampfModusScreen` (dort ersetzt er die schlichte graue Boden-Trennlinie durch eine Arena-Buehnen-Optik mit dem Horizont exakt auf `kGroundY`) und `BootScreen` (siehe naechster Punkt). Liegt bewusst hinter Icons/Charakter/Panels (wird vor diesen gezeichnet), damit die Lesbarkeit fuer die junge Zielgruppe nicht leidet.
    - **Startlogo (Nutzerwunsch "80s neon synth Schriftzug 'Henri & Theo'"):** `BootScreen` zeigt jetzt vor der SD-/Profil-Initialisierung fuer `kSplashDurationMs` (1800ms) einen Neon-Schriftzug vor dem `RetroBackdrop`-Hintergrund - simulierter "Glow" durch mehrere leicht versetzte, gedimmte Textkopien (`theme::kAccentPink`) rings um den Schriftzug plus eine Chrom-/Versatz-Kopie in Cyan unter der weissen Hauptschrift (M5GFX bietet auf `M5.Display` keine echte Transparenz/Weichzeichnung fuer einen "richtigen" Bloom-Effekt). Text ist die Konstante `kLogoText` in `BootScreen.cpp`, dort bei Bedarf anpassbar. `update()` musste dafuer von "SD-Init sofort im ersten Frame" auf "erst nach Ablauf der Splash-Dauer" umgestellt werden, sonst waere das Logo praktisch nur einen Frame sichtbar gewesen.
    - **Power-Taste: Ausschalten statt Reset (Nutzerwunsch "kein Weg um Fortschritt zurueckzusetzen ausser SD-Karte loeschen"):** siehe Korrektur weiter oben in diesem Punkt - `ESP.restart()` wurde durch `M5.Power.powerOff()` ersetzt.
    - **SD-Karte weiterhin "nicht erkannt" nach dem SPI-Bus-Fix (128GB-SDXC-Karte, FAT32) - GEKLAERT:** Diagnose ueber die neu ergaenzte Serial-Ausgabe (siehe unten) zeigte `f_mount failed: (13) There is no valid FAT volume` - die Karte wurde also am SPI-Bus korrekt erkannt (SPI-Bus-Fix greift), FatFs fand aber keine gueltige FAT-Struktur. Ursache letztlich die 128GB-Karte selbst (SDXC/Formatierung), nicht ein Firmware-Fehler: eine andere (kleinere) SD-Karte funktionierte auf Anhieb. Die dafuer ergaenzten diagnostischen/defensiven Massnahmen bleiben trotzdem wertvoll fuer kuenftige Karten-Probleme: (1) `main.cpp::setup()` startet jetzt `Serial.begin(115200)`, `BootScreen` protokolliert `SD.begin()`-Erfolg plus `SD.cardType()`/`SD.cardSize()` darueber (`pio device monitor`); (2) `BootScreen` haengt bei einem SD-Fehlschlag nicht mehr still, sondern zeigt eine deutliche, per `M5.Display.setBrightness(200)` unabhaengig vom Nachtmodus garantiert helle Fehlermeldung mit Troubleshooting-Hinweisen und versucht `SD.begin()` automatisch alle `kSdRetryIntervalMs` (2500ms) erneut. Zusaetzlich prueft `ProfileSetupScreen::commitSelection()` jetzt ebenfalls den Rueckgabewert von `profilestore::save()` (dieselbe Fehlerklasse wie beim PIN-Aendern) und zeigt bei einem Fehlschlag eine Fehlermeldung statt so zu tun, als waere das Kind-Profil erfolgreich angelegt worden.
    - **Weitere Runde nach erstem echtem Spielen:** Quiz-Fragen/Antworten liefen bei laengerem Text ueber den Bildschirmrand hinaus - neues `src/core/TextFit.*` (misst via `textWidth()`/`fontHeight()` und bricht um/verkleinert automatisch statt abzuschneiden), eingebunden in `TaskScreen`. Simon-Says (`GedaechtnisScreen`) zeigt einen schwarzen Punkt statt eines gegen helle Zonenfarben schlecht sichtbaren weissen Aufblinkens. Neues `src/core/Haptics.*` (kurze, nicht blockierende Vibration ueber `M5.Power.setVibration()`, per Timer screen-unabhaengig aus `main.cpp::loop()` wieder abgeschaltet) fuer Treffer-/Erfolgs-/Fehler-Feedback in allen Spielen sowie im Aufgaben-/Merk-Modus. `HomeScreen`-Charakter bekommt einen sanften, stark gedaempften IMU-Versatz (`updateCharacterDrift()`) - dafuer musste der Screen von "1x/Sekunde redraw" auf "jeden Frame redraw, Blinzel-Tick separat auf 1s-Kadenz" umgestellt werden. Untere Icon-Leiste von 40 auf 32px verkleinert. Neue `drawBatteryIndicator()` zeigt dauerhaft Akku-Symbol+Prozent neben der Uhrzeit statt nur einer Warnung bei kritischem Stand. Neues zehntes Spiel `LabyrinthScreen` (Nutzerwunsch: "als ersten einfachsten Game ein Ball-Labyrinth einbauen") - Neigen des Geraets treibt eine Kugel per Kreis-vs-Rechteck-Kollision durch einen simplen "S"-Wandparcours zu einem Ziel; steht als einfachstes Spiel an erster Stelle im (jetzt 3×4 statt 3×3) Spiele-Menue, freigeschaltet ab "Baby" wie Snake (eine noch frühere Freischaltung waere ueber `HomeScreen` gar nicht erreichbar, siehe Kommentar in `GamesMenuScreen.cpp`). Charakter-Trait-Farben (Haut/Haare/Kleidung) liessen sich laut Nutzer nicht sichtbar aendern - Code-Review von `CharacterCustomizeScreen`/`CharacterRenderer::applyTraitColors()` fand keinen Fehler (Trait-Farben werden bei jedem `draw()` frisch aus dem live `Profile` angewendet, Markerfarben zwischen `CharacterTraits.h` und `tools/generate_sprites.py` stimmen ueberein, die im Repo mitgelieferten PNGs enthalten nachweislich Markerpixel) - wahrscheinlichste Ursache sind veraltete Sprite-Dateien auf der SD-Karte von vor der Marker-Farben-Umstellung; kein Code-Fix vorgenommen, Empfehlung: `sdcard/sprites/character/` erneut auf die Karte kopieren.
18. **Zweite Hardware-Test-Rueckmeldung – Power-Boot-Bug + vollstaendige Grafik-Ueberarbeitung (Nutzerwunsch: "Bildschirm geht nach dem Anschalten kurz darauf wieder aus...vollstaendiges Grafikdesign wie bei Super Nintendo...komplexere Grafiken, Hintergruende...keine rudimentaeren Darstellungen mehr...optimiere Grafik maximal"):**
    - **Display ging nach dem Einschalten kurz darauf wieder aus:** root-caused auf ein AXP-Power-Chip-Verhalten - der physische Tastendruck, der das Geraet ueberhaupt erst einschaltet, setzt ein "Taste gedrueckt"-IRQ-Bit, das beim allerersten `M5.update()`-Aufruf in `loop()` noch nicht abgeklungen war. `M5.BtnPWR.wasClicked()` erkannte dadurch genau diesen Einschalt-Druck faelschlich als eigenstaendigen Kurz-Klick und schaltete das Display sofort wieder ab (`displayManuallyOff = true`). Behoben durch eine ca. 2,5 Sekunden lange "Boot-Schonfrist" (`kPowerButtonBootGraceMs`, `bootMs`) in `main.cpp::loop()`, waehrend der sowohl der Kurz-Klick- als auch der Lang-Druck-Pfad der Power-Taste ignoriert werden.
    - **Neues gemeinsames Grafik-Toolkit `src/core/GfxKit.*`:** ergaenzt `RetroBackdrop` um wiederverwendbare, reine Zeichen-Primitiven ohne neue Bild-Assets - `verticalGradient()` (Farbverlauf-Fuellung), `bevelPanel()` (erhabenes/eingedruecktes Panel mit heller/dunkler Kante, SNES-Button-Look), `starfield()` (deterministisches, nicht flackerndes Sternenfeld ueber eine Index-Hash-Formel statt echtem RNG - wichtig, da mehrere Screens jeden Frame neu zeichnen), `shinyBall()` (Kugel mit Glanzlicht+dunklerem Rand), `lighten()`/`darken()`/`lerp()` als Farb-Hilfsfunktionen auf RGB565-Ebene.
    - **HomeScreen:** flache Ein-Farb-Flaeche ersetzt durch einen echten Tag/Nacht-Szenenwechsel - Himmel-Farbverlauf (waermer/dunkler tagsueber Richtung Horizont, kuehl+Sternenfeld statt Sonne bei Nacht, `retrobackdrop::drawSynthwaveGrid()` bekam dafuer einen neuen `showSun`-Parameter), weicher Ellipsen-Bodenschatten unter der Figur (`drawGroundShadow()`), Status-/Bottom-Bar mit Farbverlauf bzw. je Icon-Zone einem eigenen gebevelten "Konsolen-Tasten"-Panel (`drawBottomBarTile()` - gesperrte Spiele-Zone bei Nacht/ohne Zeitguthaben wirkt sichtbar eingedrueckt statt nur ausgegraut).
    - **GamesMenuScreen:** Farbverlaufs-Himmel + `starfield()` hinter dem bestehenden Synthwave-Gitter, jede Spiel-Kachel nutzt jetzt `bevelPanel()` statt einer flachen `fillRoundRect()`-Flaeche (erhaben bei freigeschalteten, eingedrueckt bei gesperrten Spielen).
    - **Alle 10 Spiele grafisch ueberarbeitet:** Snake (Wiesen-Schachbrettboden, gebevelte Koerper-Segmente, Apfel mit Glanzlicht+Blatt), Tetris (Verlaufs-"Brunnen" statt Schwarzflaeche, gebeveltes Rahmenpanel, jeder Block/jedes fallende Teil gebevelt), Puzzle (gebevelte Kacheln), Space Invaders (Sternenfeld-Weltraumhintergrund, detailliertere Alien-/Schiff-Silhouette statt Flat-Rechtecke), Moorhuhn-Jagd (Himmel-Verlauf+Sonne+Wolken+Wiesenboden statt Flat-Gelbgruen), Pinball (dunkler Tisch-Verlauf+gebevelte Seitenbanden, Bumper/Ball als Glanzlicht-Kugeln), Basketball (Parkett-Verlauf, gebeveltes Backboard, Ball mit Glanzlicht+zwei Naht-Linien), Fussball (Rasenstreifen-Muster, Tor mit Netzraster statt leerem Rahmen, gebeveltes Torwart-Panel), Kampf-Modus (gebevelte Rumpf-Panels+Bodenschatten je Kaempfer, gebevelte Energiebalken-Tracks mit Farbverlaufs-Fuellung, Tasten als Glanzlicht-Kreise), Ball-Labyrinth (Verlaufs-Spielfeldboden, gebevelte Waende, mehrschichtiger Glanzring als Ziel, Kugel mit Glanzlicht).
19. **Dritte Hardware-Test-Rueckmeldung – Gyro-Inversion, Ball-Tunneling, UI-weiter Grafik-Rollout (Nutzerwunsch: "Gyrobewegungen sind invertiert...designe das komplette UI so professionell aussehend wie moeglich...90er-Jahre-Videogames...schnelle Baelle gehen einfach durch die Wand"):**
    - **Gyro-/Neigungssteuerung invertiert:** `HomeScreen::updateCharacterDrift()`, `MoorhuhnJagdScreen::updateCrosshair()`, `PinballScreen::handleInput()` und `LabyrinthScreen::updatePhysics()` nutzten `imuData.accel.x`/`.y` mit einem Vorzeichen, das auf echter Hardware zu einer dem tatsaechlichen Neigen entgegengesetzten Reaktion fuehrte. Behoben durch Vorzeichenumkehr an allen vier Stellen.
    - **Schnelle Baelle "tunnelten" durch Waende/Bumper:** `LabyrinthScreen::updatePhysics()` und `PinballScreen::updatePhysics()` bewegten die Kugel bislang in einem einzigen Schritt um `geschwindigkeit * deltaMs` und prueften Kollisionen erst danach - bei hoher Geschwindigkeit (v. a. nach Flipper-Boost in Pinball) konnte dieser Schritt eine duenne Wand oder einen Bumper komplett ueberspringen, ohne dass die Kollisionspruefung je "getroffen" hat (klassisches diskretes-Physik-Tunneling-Problem). Behoben durch Sub-Stepping: die Bewegung + Kollisionspruefung wird pro Frame in bis zu 8 Teilschritte von maximal 4px aufgeteilt (Schrittzahl dynamisch aus der aktuellen Geschwindigkeit berechnet), Schwerkraft/Reibung/Neigungsbeschleunigung bleiben weiterhin nur 1x pro Frame angewendet, um das bestehende Geschwindigkeitsverhalten nicht zu veraendern.
    - **UI-weiter Rollout der GfxKit-Grafiküberarbeitung (Nutzerwunsch: "designe das komplette UI so professionell aussehend wie moeglich"):** die in Punkt 18 auf Home-Screen/Spiele-Menue/alle 10 Spiele angewandte Optik (Farbverlaeufe statt Flat-Fill, `gfxkit::bevelPanel()` statt `fillRoundRect()`/`drawRoundRect()` fuer Buttons/Kacheln/Listenzeilen, `gfxkit::shinyBall()` fuer Kreis-Elemente) wurde auf alle uebrigen Screens ausgeweitet, die vorher noch unveraendert waren: `AlltagMenuScreen`/`SubjectSelectScreen` (Farbverlaufs-Himmel+Sternenfeld+gebevelte Kacheln, analog `GamesMenuScreen`), `ClockScreen`, `TimerScreen`, `ChecklistScreen`, `WebSyncScreen`, `BirthdayScreen`, `CharacterCustomizeScreen`, `SettingsScreen`, `DateTimeSetScreen`, `PinEntryScreen`, `BootScreen` (Logo- und SD-Fehlerscreen), `ProfileSetupScreen`, `TaskScreen`, `GedaechtnisScreen` (Memory + Simon-Says) und `SteckbriefScreen`.
20. **Vierte Hardware-Test-Rueckmeldung – maximale Spielegrafik-Ueberarbeitung (Nutzerwunsch: "verbessere Spielegrafik...hole das Maximum aus der Hardware...maximal hochwertig...mit Backgrounds"):**
    - **`src/core/GfxKit.*` erweitert:** `starfield()` bekam optionale `xOffset`/`yOffset`-Parameter (Default 0, bestehende Aufrufe unveraendert kompatibel), damit sich Punktefelder gezielt in einem begrenzten Bereich platzieren lassen (z. B. innerhalb eines Tetris-Spielfelds oder als Zuschauer-Andeutung ueber einem Kampf-Modus-Boden) statt nur ab (0,0). Neue Funktion `hillsSilhouette()` fuer deterministische Huegel-/Bergsilhouetten (feste Sinus-basierte Formel je Huegel-Index, kein echtes Rauschen - analog zur bestehenden `starfield()`-Begruendung, bleibt bei jedem Redraw identisch).
    - **Alle 10 Spiele bekamen eine eigene, mehrschichtige Szenerie statt eines einfachen Verlaufs:** Tetris (Sternenfeld im Brunnen via neuer `starfield()`-Offsets, Ziegel-/Mauerpaneele in den Seitenbereichen links/rechts vom Spielfeld), Space Invaders (Ringplanet per zweifach uebermaltem Ellipsen-Trick + Nebelwolke aus ueberlappenden gedimmten Ellipsen in der ruhigen Zone zwischen Alien-Formation und Schiff), Moorhuhn-Jagd (`hillsSilhouette()` + Baumreihe zwischen Himmel und Wiese), Kampf-Modus (Sonne durch Nacht-Buehne ersetzt: Mond, `hillsSilhouette()`-Bergkette, Zuschauer-/Lichter-Andeutung als Punkteband knapp ueber dem Boden via `starfield()`-yOffset - `retrobackdrop::drawSynthwaveGrid()` laeuft weiterhin fuer das Buehnenboden-Gitter, jetzt mit `showSun=false`), Basketball (zwei weiche Hallenscheinwerfer-Lichtflecken, Parkett-Maserungslinien, Freiwurf-Zonen-Rahmen), Fussball (bunte Stadion-Werbebanden-Bloecke links/rechts vom Tor), Ball-Labyrinth (versetztes Ziegel-/Steinboden-Fugenraster als Dungeon-Boden), Puzzle (vier Ecken-Vignette + goldener, gebevelter Galerie-Bilderrahmen um das Spielfeld), Pinball (Lichterketten-Reihe unter der Kopfzeile + weicher Jackpot-Lichtschein hinter den Bumpern), Snake (Hecken-Beeteinfassung am Spielfeldrand, rein dekorativ - Schlange/Apfel duerfen weiterhin bis an den Rand und werden darueber gezeichnet).
    - **Selbstgefundener Bug (vor dem Commit behoben):** `TetrisScreen::drawSidePanel()` wurde zunaechst faelschlich als `const`-Methode deklariert, obwohl sie ueber das Member `canvas_` zeichnet (`canvas_.drawLine()`) - `M5Canvas`s Zeichenfunktionen sind nicht `const`, eine `const`-Memberfunktion haette hier nicht kompiliert (nur Zugriff auf `const`-qualifizierte Members waere erlaubt gewesen). Beim Selbst-Review vor dem Commit anhand desselben Musters wie `GamesMenuScreen::drawIcon()` (dort unproblematisch, da dort ueber das globale `M5.Display` statt ein eigenes Member gezeichnet wird) gefunden und korrigiert.

21. **Fuenfte Hardware-Test-Rueckmeldung – Kleidungsfarbe weiterhin nicht aenderbar (zweite Meldung, Nutzerwunsch: "Farben der Kleidung lassen sich nach wie vor nicht aendern"):** erneutes, gruendlicheres Code-Review von `CharacterRenderer::applyTraitColors()`, `CharacterCustomizeScreen` und `tools/generate_sprites.py` findet weiterhin keinen Logikfehler - die Markerfarben-Pruefung ist fuer Haut/Haar/Kleidung symmetrisch identisch implementiert (`px == traits::kSkinMarker/kHairMarker/kClothMarker`, exakter RGB565-Vergleich), `CharacterTraits.h`s `kClothMarker` und `generate_sprites.py`s `MARKER_CLOTH` stimmen ueberein (`(255,255,0)`), die Touch-Zonen aller drei Trait-Zeilen in `CharacterCustomizeScreen` nutzen denselben Code-Pfad (`cycleIndex()`) und unterscheiden sich nur in der Y-Position. Zusaetzlich diesmal verifiziert statt nur behauptet: `tools/generate_sprites.py` neu ausgefuehrt und `git status` auf `sdcard/sprites/character/` geprueft - keine Aenderung, die im Repo mitgelieferten PNGs sind also byte-identisch zu frisch generierten Dateien mit korrekten Markerpixeln (u. a. `baby_idle1.png`: 85 Haut-, 28 Haar-, 104 Kleidungs-Markerpixel). Damit ist ein Fehler in den mitgelieferten Assets/im Code weiterhin nicht nachweisbar - **neu diesmal:** `CharacterRenderer::applyTraitColors()` zaehlt jetzt beim Ersetzen die getroffenen Pixel je Markerfarbe und loggt einmalig ueber `Serial`, wenn ein Sprite null Kleidungs-Markerpixel enthaelt, obwohl Haut-/Haar-Marker gefunden wurden - das waere der eindeutige Beweis fuer eine veraltete Sprite-Datei auf der SD-Karte (aus der Zeit vor der Marker-Farben-Umstellung, mit fest einprogrammierter statt Marker-Kleidungsfarbe) und liefert beim naechsten `pio device monitor`-Test endlich eine harte Diagnose statt einer Vermutung.

22. **Sechste Hardware-Test-Rueckmeldung – Uhrzeit-/Spielzeit-Verlust, Pinball-Ueberarbeitung, Moorhuhn-Timer, Tools-Grafik (Nutzerwunsch: "gaming timer funktioniert nicht, Uhrzeit geht verloren wenn man das Geraet ausschaltet...pinball Ball geht durch die Balken durch...pinball komplett ueberarbeiten...bei Moorhuhn 60 Sekunden Timer einbauen"):**
    - **Uhrzeit-/Spielzeit-Verlust root-caused:** manche Core2-Boards haben keinen ausreichenden RTC-Backup-Akku - seit dem Wechsel auf echtes `M5.Power.powerOff()` (Punkt 17/18) verliert die RTC beim Abschalten ihre Pufferspannung und faellt auf ihr Chip-Default-Datum zurueck. `PlaytimeAccount::rolloverIfNewDay()` erkennt dadurch jeden Neustart faelschlich als neuen Tag und loescht verdiente Spielzeit - genau das vom Nutzer gemeldete "Spielzeit-Timer funktioniert nicht". Neues `src/core/RtcBackupService.*`: sichert die RTC-Zeit atomar auf die SD-Karte (periodisch alle 5 Minuten aus `main.cpp::loop()` sowie unmittelbar vor `M5.Power.powerOff()`) und stellt sie in `BootScreen::update()` wieder her, falls die RTC beim Boot eine unplausible Zeit (Jahr < `config::kRtcPlausibleMinYear`, neu nach `config.h` ausgelagert und von `NightModeService` mitgenutzt statt dort lokal dupliziert) zeigt. Keine perfekte Loesung (waehrend der Abschaltzeit tatsaechlich vergangene Zeit bleibt unbekannt), behandelt aber den Alltagsfall "kurz aus, wieder an" korrekt.
    - **Pinball komplett ueberarbeitet (Nutzerwunsch: "pinball Ball geht durch die Balken durch und prallt nicht ab...pinball komplett ueberarbeiten und verbessern"):** die Flipper waren bisher nur eine reine Trefferzone (feste Boost-Geschwindigkeit, wenn Ball-Position in einem Rechteck lag UND der Flipper gerade aktiv gehalten wurde) - bei inaktivem Flipper oder ausserhalb der Zone flog die Kugel ungebremst hindurch, exakt das gemeldete Symptom. Die Flipper sind jetzt echte Kollisionsobjekte: `resolveFlipperCollision()` macht Kreis-vs-Liniensegment-Kollision (dieselbe Projektions-Formel wie `LabyrinthScreen::resolveWallCollision()`, hier fuer ein Segment statt ein Rechteck) zwischen einem festen Drehpunkt und einer von zwei Endpositionen (Ruhe-/Aktiv-Winkel, `leftFlipperTip()`/`rightFlipperTip()`), die in JEDEM Sub-Schritt der Physik greift - unabhaengig vom Flipper-Zustand blockt der Flipper jetzt immer, ist er zusaetzlich gerade aktiv gehalten, gibt der erste Kontakt seit dem Aktivieren (per bestehendem `leftFlipperBoosted_`/`rightFlipperBoosted_`-Flag genau einmal pro Tastendruck) einen kraeftigen "Flick"-Schub obendrauf. `drawFlipper()` zeichnet den Flipper dafuer jetzt als rotiertes, gebeveltes Viereck (zwei `fillTriangle()` ueber senkrecht zur Balken-Achse versetzte Eckpunkte) statt der alten festen horizontalen Leiste. Zusaetzlich mehr Tisch-Elemente: `Bumper` bekam `points`/`color`-Felder, zwei kleine pinke Slingshot-Kicker neben den Flippern (5 Punkte) und ein goldenes, hoeher bepunktetes Bonus-Ziel oben mittig im Jackpot-Lichtschein (25 Punkte) ergaenzen die drei bisherigen Rundbumper (10 Punkte).
    - **Moorhuhn-Jagd: feste 60-Sekunden-Runde statt unbegrenztem Score (Nutzerwunsch: "bei Moorhuhn 60 Sekunden Timer einbauen...wieviele Abschuesse in dieser Zeit inkl. Highscore"):** neuer Rundenzustand (`roundElapsedMs_`/`roundOver_`, `kRoundDurationMs = 60000`) - `score_` zaehlt weiterhin die Treffer, jetzt aber innerhalb der 60-Sekunden-Runde statt nur durch das Spielzeitguthaben begrenzt; Restzeit wird oben rechts angezeigt (aufgerundet, damit die Anzeige mit "60" statt sofort "59" startet). Laeuft die Zeit ab, friert `endRound()` das Spielfeld ein (`roundOver_ = true`), speichert den Rundenendstand ueber `highscorestore::saveIfHigher()` (bereits vorhandener Key "moorhuhn", vorher nur beim vorzeitigen Verlassen genutzt) und zeigt ein Ergebnis-Overlay ("Zeit abgelaufen!", Abschuesse, Highscore, "Tippen fuer neue Runde") - Antippen startet ueber `resetGame()` (jetzt zusaetzlich `roundElapsedMs_`/`roundOver_` zuruecksetzend) eine neue Runde, exakt das vom Nutzer beschriebene "Abschuesse in dieser Zeit inkl. Highscore".
    - **Tools-Menue (Alltagsfunktionen) auf dasselbe Grafik-Niveau wie die Spiele gehoben (Nutzerwunsch: "grafiken im tools menue ebenfalls maximal verbessern"):** die 10 Spiel-Screens hatten bereits GfxKit-Himmel/Sternenfelder/Bevel-Panels (Punkt 20), die 6 Alltagsfunktionen-Screens blieben bisher bei flachen Ein-Farb-Verlaeufen. Jetzt: `AlltagMenuScreen` bekam eine `hillsSilhouette()`-Skyline zwischen Sternenfeld und Synthwave-Gitter fuer mehr Tiefe; `ClockScreen` unterscheidet jetzt wie `HomeScreen` zwischen Tag (warmer Verlauf) und Nacht (dunkler Verlauf + Sternenfeld, ueber `nightmodeservice::isNight()`) statt eines immer gleichen Hintergrunds; `TimerScreen`/`ChecklistScreen` bekamen ein dezentes Sternenfeld unterhalb der Titelleiste; `SteckbriefScreen` zeigt vor jeder Statuszeile eine farbige Glanzlicht-Kugel als Aufzaehlungspunkt (`gfxkit::shinyBall()`, je Zeile eine andere Akzentfarbe) statt reinem Fliesstext; `CharacterCustomizeScreen` bekam einen Buehnenlicht-Effekt (drei ineinanderliegende, zur Mitte hin hellere Kreise) hinter der Charakter-Vorschau; `BirthdayScreen` bekam festes (nicht zufaelliges) Konfetti in Akzentfarben hinter der Geburtstagstorte. Alle Ergaenzungen sind rein additiv (bestehende Touch-Zonen/Layout-Konstanten unveraendert) und wie ueberall in GfxKit deterministisch statt `rand()`-basiert, damit ein Screen mit periodischem Redraw nicht sichtbar flimmert.
...
- Aufgaben-Content ist erweitert (**Update, nach Nutzerwunsch "deutlich mehr Aufgaben...für alle Stufen, sollen immer schwieriger werden"**: 45–79 Fragen je Datei unter `sdcard/tasks/`, total 376 statt vorher 185, gegenüber urspruenglich 8–10) und damit spürbar weniger repetitiv als der erste Platzhalter-Umfang, aber weiterhin selbst verfasst statt curriculumsgeprüfter Lehrplan-21/Passepartout-Stoff – Content-Pflege/-Erweiterung bleibt der grösste Zeitfaktor auf lange Sicht (siehe Abschnitt 10, Scope/Ressourcen-Hinweis). Neu deckt der Content auch `schwierigkeit` 4-5 ab (vorher ueberwiegend 1-3), damit `DifficultyTracker`s monatlicher Auto-Anstieg der Obergrenze (Abschnitt 8.4) tatsaechlich schwierigere Fragen zu servieren hat. Mathe-Fragen wurden dafuer programmatisch generiert (Python-Skript berechnet die korrekte Antwort direkt aus der Aufgabe, kein manuelles Eintippen) - dabei zunaechst ein Bug in der Mischen-der-Antwortoptionen-Logik gefunden und korrigiert, der bei einem Teil der Fragen den falschen Index als "richtig" markiert haette (Permutation vs. inverse Permutation verwechselt); danach jede Frage sowohl per Assertion beim Generieren als auch unabhaengig per Nachrechnen aus dem Fragetext verifiziert. Zusaetzlich: `SpacedRepetitionStore`s Box-1-Intervall (frisch falsch beantwortet) war faelschlich 1 Tag statt 0 - eine falsche Antwort konnte dadurch in derselben Sitzung nie wiederkehren, nur am naechsten Kalendertag (Nutzer-Feedback: "falsche Antworten oefters wiederholt"). Behoben auf 0 Tage (noch am selben Tag wieder faellig) plus zusaetzliche Gewichtung in `TaskEngine::pickNextTask()`, die Box-1-Items unter mehreren gleichzeitig faelligen Items bevorzugt.
