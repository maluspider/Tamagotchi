...
17. **Erste echte Hardware-Testrunde (Abschnitt 5/6/9/11, neu) – auf einem echten Core2 gefundene und behobene Laufzeit-Fehler:**
    ...
    - **SD-Karte weiterhin "nicht erkannt" nach dem SPI-Bus-Fix (128GB-SDXC-Karte, FAT32):** noch ungeklaerte Ursache - moeglich sind SDXC->32GB-Kompatibilitaetsprobleme der Arduino-ESP32-`SD`-Bibliothek ueber SPI (bekannte Problemklasse, unabhaengig vom bereits behobenen SPI-Bus-Pin-Fehler) oder eine nicht-standardkonforme FAT32-Formatierung auf einer >32GB-Karte. Da kein Testgeraet zur Verfuegung steht, wurden statt eines Rate-Fixes zwei diagnostische/defensive Massnahmen ergaenzt: (1) `main.cpp::setup()` startet jetzt `Serial.begin(115200)`, `BootScreen` protokolliert `SD.begin()`-Erfolg plus `SD.cardType()`/`SD.cardSize()` darueber (`pio device monitor`), damit ein erneuter Fehlschlag konkrete Diagnosedaten statt eines reinen "geht nicht" liefert; (2) `BootScreen` haengt bei einem SD-Fehlschlag nicht mehr still, sondern zeigt eine deutliche, per `M5.Display.setBrightness(200)` unabhaengig vom Nachtmodus garantiert helle Fehlermeldung mit Troubleshooting-Hinweisen und versucht `SD.begin()` automatisch alle `kSdRetryIntervalMs` (2500ms) erneut - kein manueller Neustart mehr noetig, sobald die Karte korrekt sitzt. Zusaetzlich prueft `ProfileSetupScreen::commitSelection()` jetzt ebenfalls den Rueckgabewert von `profilestore::save()` (dieselbe Fehlerklasse wie beim PIN-Aendern) und zeigt bei einem Fehlschlag eine Fehlermeldung statt so zu tun, als waere das Kind-Profil erfolgreich angelegt worden.
