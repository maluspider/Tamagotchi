...
**Update (Sprite-Grafik):** Der Home-Screen-Charakter nutzt jetzt echte 32×32-PNG-Sprites von der SD-Karte (`sdcard/sprites/character/<stufe>_<idle1|idle2|sad>.png`, 6 Stufen × 3 Varianten = 18 Dateien) statt der Platzhalter-Grafik. Da in dieser Session weder ein klassisches Zeichenprogramm noch handgemalte Artworks verfügbar waren, wurden die Sprites **prozedural als echte Bilddateien erzeugt**: `tools/generate_sprites.py` baut jede Form pixelweise aus Ellipsen/Linien auf einem 32×32-Raster zusammen (keine PIL-Anti-Aliasing-Kanten, dadurch echter Pixel-Art-Look) und exportiert sie als PNG mit transparentem Hintergrund. Ergebnis: begrenzte, kräftige Farbpalette, durchgängige dunkle Outlines, chibi-artiger Rundkörper – stilistisch an die Vorgaben oben angelehnt, aber bewusst simpler als handgezeichnete 90er-Jahre-Referenzsprites (keine Schattierungs-/Dithering-Finesse, keine Mehrfach-Frame-Laufanimation). Jede Stufe hat drei Varianten: `idle1`/`idle2` (Blinzeln, wechselt bei jedem Redraw-Tick) und `sad` (bei Inaktivität, siehe Abschnitt 9). `HomeScreen::drawSpriteCharacter()` lädt die passende Datei über `M5GFX::drawPngFile(SD, ...)`; existiert sie nicht (SD-Karte fehlt, Datei nicht kopiert), fällt der Code automatisch auf `drawPlaceholderCharacter()` zurück – die Platzhalter-Grafik ist also nicht entfernt, sondern bleibt als Fallback aktiv. **Offen:** Spiel-interne Charaktergrafiken (Kampf-Modus 48–64 px, Puzzle-Motive) nutzen weiterhin keine Sprites, siehe Abschnitt 16. Optik-Anpassung (Kleidung/Haarfarbe) ist mit diesem Sprite-System technisch vorbereitbar (weitere Dateivarianten pro Kombination), aber noch nicht umgesetzt.
...
- **Nicht umgesetzt** (Zeitbudget, siehe Abschnitt 16): Taschenrechner, Geburtstags-/Ereignis-Countdown, mehrere Wecker gleichzeitig (aktuell weiterhin nur einer, wie in Phase 1 angelegt).
...
- Home-Screen-Charakter nutzt seit dem Sprite-Update (Abschnitt 4) echte, prozedural generierte PNG-Sprites (`tools/generate_sprites.py`) statt der reinen Formen-Platzhalter. Offen bleibt: (a) Kampf-Modus-Sprites (48–64 px, mit Angriffs-/Treffer-Frames) und Puzzle-Bildmotive nutzen weiterhin keine Sprites; (b) die generierten Sprites sind einfacher als handgezeichnete 90er-Jahre-Referenzsprites (kein Dithering, keine Lauf-/Angriffsanimation) – bei Bedarf durch selbst gezeichnete, KI-generierte oder freie Assets im 90er-Stil ersetzbar, ohne Codeänderung (nur Dateien unter `sdcard/sprites/character/` austauschen, gleiches Namensschema `<stufe>_<idle1|idle2|sad>.png`).
...
- Taschenrechner und Geburtstags-/Ereignis-Countdown aus Abschnitt 11 ("Zusaetzliche Vorschlaege") sind nicht umgesetzt.
